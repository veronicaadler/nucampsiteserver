...
campsiteRouter
  ...
  .put(authenticate.verifyUser, (req, res, next) => {
    //return campsite by the id in the url
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        //testing for the values coming in on request
        console.log(req.user._id);
        console.log(campsite.comments.id(req.params.commentId).author);
        //checking if we found a campsite and campsite id
          if (campsite && campsite.comments.id(req.params.commentId)) {
        //checking if the author of the comment is the one requesting to change the comment
            if ((campsite.comments.id(req.params.commentId).author).equals(req.user._id)) {
                ...
                campsite
                    ...
                    .catch((err) => next(err));
            } else { //if the one requesting to change comment is not the author
            err = new Error("You are not the author of this comment.");
            err.statuscode = 403; //Forbidden status code
            return next(err);
            }
          } else if (!campsite) { //if no campsite is returned
            err = new Error(`Campsite ${req.params.campsiteId} not found`);
            err.status = 404; //Not Found status code
            return next(err);
          } else { //if no comment is returned
            err = new Error(`Comment ${req.params.commentId} not found`);
            err.status = 404; //Not Found status code
            return next(err);
          }
